...
class GNRMLP(nn.Module):
    def __init__(self, opt, D=8, W=256, input_ch=3, input_ch_atts=3, output_ch=4,
        ...
        self.pose_embeder = PositionalEncoding(self.input_ch_pos_enc, num_freqs=pose_freqs, min_freq=spatial_freq * 0.1,
                                               max_freq=spatial_freq * 10)
        self.att_embeder = SphericalHarmonics(d=self.input_ch_atts) if self.use_sh else PositionalEncoding(
        ...
        self.alpha_linears = nn.ModuleList(
            ...
            [nn.Linear(W + self.pose_embeder.out_dim + self.input_ch_smpl, W) if i in self.skips else nn.Linear(W, W)
        ...
        if self.use_attention:
            self.value_linears = nn.ModuleList(
                [nn.Linear(self.pose_embeder.out_dim + self.att_embeder.out_dim + W, W // 4),
        ...
        if self.use_occ_net:
            self.occ_linears = nn.ModuleList(
                [nn.Linear(self.input_ch_smpl + 6 + self.input_ch_feat, W // 4), nn.Linear(W // 4, W // 16),
    ...
    def forward(self, x, attdirs=None, alpha_only=False, smpl_vis=None):
        ...
        torch.Size([8590, 3])
        torch.Size([8590, 1])
        torch.Size([8590, 4, 269]) 3 7 131
        '''
        #print(x.shape, self.input_ch_pos_enc, self.input_ch_smpl, self.input_ch_feat)
        input_pts, input_smpl, input_feats = torch.split(x, [self.input_ch_pos_enc, self.input_ch_smpl,
                                                             self.input_ch_feat], dim=-1)
        unqiue_pts = input_pts[:, 0]
        unqiue_smpl = input_smpl[:, 0] if self.use_smpl else torch.zeros([input_pts.shape[0], 0], dtype=torch.float32,
                                                                         device=input_pts.device)
        input_pts = input_pts.view([-1, self.input_ch_pos_enc])
        input_smpl = input_smpl.view([-1, self.input_ch_smpl]) if self.use_smpl else torch.zeros(
            [input_pts.shape[0], 0], dtype=torch.float32, device=input_pts.device)
