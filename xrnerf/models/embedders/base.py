...
class BaseEmbedder(nn.Module):
    def __init__(self, i_embed=0, multires=10, multires_dirs=4, input_ch=3, **kwargs):
        ...
        if i_embed==-1:
            ...
            self.embed_fns, self.embed_ch = self.create_embedding_fn(multires, input_ch=input_ch)
            self.embed_fns_dirs, self.embed_ch_dirs = self.create_embedding_fn(multires_dirs, input_ch=input_ch)

    def create_embedding_fn(self, multires, input_ch=3, cat_input=True, log_sampling=True, periodic_fns=[torch.sin, torch.cos]):
        num_freqs = multires
