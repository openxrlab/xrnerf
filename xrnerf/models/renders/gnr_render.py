...
class GnrRenderer:
    def __init__(self, opt, nerf_fine=None, projection='perspective', vgg_loss=None, threshold=0.5):
        self.opt = opt
        self.nerf = opt.model
    ...
    def get_rays_orthogonal(self, bbox, calib):
        ...
        i, j = torch.meshgrid(torch.linspace(top, bottom - 1, int(bottom - top), device=calib.device),
                              torch.linspace(left, right - 1, int(right - left),
    ...
    def make_nerf_input(self, pts, feats, images, smpl, calibs, mesh_param, persps=None, is_train=True):
        ...
        if feats is not None:
            ...
            nerf_input = [inp[:, None, :].expand([-1, self.num_views, -1]) for inp in
                          nerf_input]  # expand each feature to num_views
            nerf_input += [latent]
    ...
    def make_nerf_output(self, nerf_output, t_vals, norm, source_rgb, is_train=True):
        ...
        weights = alpha * torch.cumprod(
            torch.cat([torch.ones((alpha.shape[0], 1), device=nerf_output.device), 1. - alpha + 1e-10], -1), -1)[:, :-1]
    ...
    def render_rays(self, ray_batch, feats, images, masks, calibs, smpl, mesh_param,
        ...
        if self.use_vh:
            ...
            row_cols = torch.cat([inside_idx.view(1, -1), torch.arange(len(inside_idx), device=pts.device).view(1, -1)],
                                 0)
            I = torch.sparse_coo_tensor(row_cols, torch.ones(len(inside_idx), dtype=pts.dtype, device=pts.device),
                                        size=(N_rays * self.N_samples, len(inside_idx)))
    ...
    def render(self, feats, images, masks, calibs, bbox, mesh_param, smpl=None, scan=None, persps=None):
        ...
        coords = torch.stack(
            torch.meshgrid(torch.linspace(0, bottom - top - 1, int(bottom - top), device=calibs.device),
                           torch.linspace(0, right - left - 1, int(right - left), device=calibs.device)),
            -1)  # (H, W, 2)

        coords = coords.view(-1, 2)  # (H * W, 2)
        select_inds = np.random.choice(coords.shape[0], size=[self.N_rand * self.vh_overhead],
        ...
        if self.use_vh:
            ...
            for i in range(0, pts.shape[0], self.chunk):
                inside, _, _ = self.inside_pts_vh(
                    torch.tensor(pts[i:i + self.chunk], dtype=torch.float32, device=device),
