...
# @Last Modified time: 2022-06-08 11:31:01
...
class SetValPipelineHook(Hook):
    ...
    def __init__(self, cfg=None, valset=None):
        self.cfg = cfg
...
class SaveTestHook(Hook):
    """save testset's render results with test poses 在每次val_step()之后调用
    用于保存test数据集的渲染图片 这些图片是有groundtruth的 可用来计算指标."""
    ...
    def after_val_iter(self, runner):
        ...
        if rank == 0:
            ...
            testset_dir = os.path.join(
                runner.work_dir, 'visualizations/testset/{}'.format(cur_iter))
