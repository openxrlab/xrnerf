...
from .utils.genebody import load_ply, load_obj_mesh, gen_cam_views
...
class GeneBodyDataset(BaseDataset):
    ...
    def get_image(self, sid, num_views, view_id=None, random_sample=False, smpl_verts=None):
        ...
        return {
            ...
            'bbox': bbox,
            'render_gt': torch.stack(gt_list, dim = 0) if not self.is_train and self.move_cam is 0 else torch.tensor([]),
            'smpl_depth': torch.stack(smpl_depth_list[:self.num_views], dim=0) if self.opt.use_smpl_depth else torch.tensor([]),
