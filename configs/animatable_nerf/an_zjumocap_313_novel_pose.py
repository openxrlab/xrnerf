...
if not os.path.exists(work_dir):
    ...
    ckpt_path = os.path.join(work_dir_pattern.format('train_pose'), 'latest.pth')
...
basedata_cfg = dict(
    ...
    num_train_pose = num_train_pose,
    training_frame=[0, num_train_pose * frame_interval], # [begin_frame, end_frame]
    novel_pose_frame=[num_train_pose * frame_interval, (num_train_pose + num_novel_pose) * frame_interval],
...
train_pipeline = [
    ...
    dict(type='CalculateSkelTransf', enable=True,), # 计算骨架变换矩阵
    dict(type='AninerfIdxConversion', enable=True,), # 变换latent index
    ...
    dict(type='DeleteUseless', enable=True, keys=['iter_n', 'cams', 'cam_inds', 'ims', 'cfg', 'data_root', 'idx', 'img_path', 'num_cams', 'parents', 'joints']),
]
