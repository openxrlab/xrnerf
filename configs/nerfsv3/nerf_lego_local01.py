...
log_level = 'INFO'
log_config = dict(interval=5,  by_epoch=False, hooks=[dict(type='TextLoggerHook')])
workflow = [('train', 5), ('val', 1)]

# runtime settings
num_gpus = 1
distributed = (num_gpus>1)  # 是否多卡，mmcv对dp多卡支持不好，故而要么单卡要么ddp多卡
work_dir = './work_dirs/nerfsv3/nerf_lego_base01/'  # noqa
...
N_rand_per_sampler = 256     # how many N_rand in get_item() function
...
model = dict(
    ...
    cfg=dict(
        ...
        chunk=1024*4, # mainly work for val_step test_step
        ),
    mlp=dict( # coarse model
        ...
        netchunk=1024*4, # number of pts sent through network in parallel;
...
testdata_cfg.update( dict(mode='test', testskip=40) )
